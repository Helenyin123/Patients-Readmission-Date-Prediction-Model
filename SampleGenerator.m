function [train_sample,train_label,test_sample,test_label] = SampleGenerator(X,label)
...
train_sample = [train1;train2;train3];
test_label = [ones(length(test1),1);2 * ones(length(test2),1); 3 * ones(length(test3),1)];
train_label = [ones(length(train1),1);2 * ones(length(train2),1); 3 * ones(length(train3),1)];